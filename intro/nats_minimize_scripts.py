        ...
        if not file.endswith("-a.pdb") and not file.endswith("-a2.pdb"):
            ...
            with open("../data/input/etc/build_nats_resiude-gap.inp") as f1:
                for line in f1.readlines():
                    ...
                    line_list.append(line)
            if file.endswith("-a.pdb"):
                with open("build_{}.inp".format(file.replace("-a.pdb", "")), "w") as f2:
                    f2.writelines(line_list)
                scripts_list.append("build_{}.inp".format(file.replace("-a.pdb", "")))
                scripts_build_list.append("build_{}.inp".format(file.replace("-a.pdb", "")))
            if file.endswith("-a2.pdb"):
                with open("build_{}.inp".format(file.replace("-a2.pdb", "")), "w") as f2:
                    f2.writelines(line_list)
                scripts_list.append("build_{}.inp".format(file.replace("-a2.pdb", "")))
                scripts_build_list.append("build_{}.inp".format(file.replace("-a2.pdb", "")))
